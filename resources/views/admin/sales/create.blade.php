...
@section('title', 'Tambah Anggota Sales Baru')

@section('content')
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-800 dark:text-slate-100">Tambah Anggota Sales Baru</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Isi formulir di bawah untuk menambahkan anggota tim sales baru.
            </p>
        </div>
        <div>
    ...
    @if ($errors->any() && !$errors->hasAny(['name', 'email', 'no_phone', 'password', 'target_sales']))
        <div class="mb-4 p-4 text-sm text-red-800 rounded-lg bg-red-100 dark:bg-red-500/20 dark:text-red-400" role="alert">
            <span class="font-medium">Oops!</span> Terjadi beberapa kesalahan dengan input Anda. Silakan periksa kembali.
        </div>
    @endif
        ...
        <form action="{{ route('admin.sales.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div>
                    ...
                    <input type="tel" name="no_phone" id="no_phone" value="{{ old('no_phone') }}" required
                ...
                <div>
                    ...
                    <input type="number" name="target_sales" id="target_sales" value="{{ old('target_sales', 0) }}" min="0" step="100000"
                ...
                {{-- Kolom Area dan Status dikomentari karena tidak ada di skema database Sales Anda --}}
                {{-- <div>
                    <label for="area" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Area Penjualan</label>
                    <input type="text" name="area" id="area" value="{{ old('area') }}"
                           class="w-full px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-300 rounded-lg shadow-sm {{ $errors->has('area') ? 'border-red-500 dark:border-red-600 focus:ring-red-500/50 focus:border-red-500' : 'border-slate-300 dark:border-slate-700 focus:ring-primary/50 focus:border-primary/50' }}">
                    @error('area') <p class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="status" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Status</label>
                    <select name="status" id="status"
                            class="w-full px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-300 rounded-lg shadow-sm {{ $errors->has('status') ? 'border-red-500 dark:border-red-600 focus:ring-red-500/50 focus:border-red-500' : 'border-slate-300 dark:border-slate-700 focus:ring-primary/50 focus:border-primary/50' }}">
                ...
                <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                    {{-- Password --}}
                    <div>
                        <label for="password" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Password <span class="text-red-500">*</span></label>
                    ...
                    {{-- Konfirmasi Password --}}
                    <div>
                        <label for="password_confirmation" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Konfirmasi Password <span class="text-red-500">*</span></label>
            ...
            <div class="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
                <div class="flex justify-end gap-3">
                    <a href="{{ route('admin.sales.index') }}" class="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-colors">
                        Batal
                    </a>
                    <button type="submit" class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg shadow-sm hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/50 transition-colors">
                        ...
                        <span>Simpan Anggota Sales</span>
                    </button>
                </div>
            </div>
        </form>
