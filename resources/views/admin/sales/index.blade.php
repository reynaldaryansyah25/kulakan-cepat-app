...
@section('title', 'Manajemen Tim Sales')
...
@section('content')
  <div class="container mx-auto px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Manajemen Tim Sales</h1>
        <p class="mt-1 text-base text-slate-600 dark:text-slate-300">
          Kelola dan pantau performa tim sales Anda
        </p>
      </div>
    ...
    <div class="mb-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="group relative overflow-hidden">
        <div
          ...
          <h3 class="text-xs font-bold text-slate-700 dark:text-slate-200">Total Tim Sales</h3>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Jumlah sales terdaftar</p>
        </div>
      </div>
      ...
      <div class="group relative overflow-hidden">
        <div
          ...
          <h3 class="text-xs font-bold text-slate-700 dark:text-slate-200">Sales Aktif</h3>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Sales yang sedang bekerja</p>
        </div>
      </div>
        ...
        <div
          ...
          <div class="mb-3 flex items-center justify-between">
            ...
            <div class="text-right">
              ...
              <p class="text-lg font-black text-slate-800 dark:text-slate-100">
                Rp {{ number_format($stats->totalTarget ?? 0, 0, ',', '.') }}
              </p>
            </div>
          </div>
          <h3 class="text-xs font-bold text-slate-700 dark:text-slate-200">Target Bulan Ini</h3>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Total target sales</p>
        </div>
      </div>
      ...
      <div class="group relative overflow-hidden">
        <div
          ...
          <div class="mb-3 flex items-center justify-between">
            <div
              class="rounded-lg bg-gradient-to-br from-orange-100 to-orange-200 p-2 dark:from-orange-500/20 dark:to-orange-600/20">
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="text-right">
              <p
                class="text-[11px] font-bold uppercase tracking-wider text-orange-600 dark:text-orange-400">
                Pencapaian
              </p>
              <p class="text-lg font-black text-slate-800 dark:text-slate-100">
                {{ number_format($stats->totalAchievement ?? 0, 0) }}%
              </p>
            </div>
          </div>
          <h3 class="text-xs font-bold text-slate-700 dark:text-slate-200">Rata-rata Pencapaian</h3>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Rata-rata performa sales</p>
        </div>
      </div>
    </div>

    {{-- Filter Tanpa Background --}}
    <div class="mb-6">
      <form action="{{ route('admin.sales.index') }}" method="GET" id="salesFilterForm">
        <div class="flex flex-col items-center gap-4 sm:flex-row">
          ...
          <div class="w-full sm:w-auto">
            <select
              name="sales_status"
              id="sales_status"
              onchange="document.getElementById('salesFilterForm').submit();"
              ...
              <option value="Aktif" {{ request('sales_status') == 'Aktif' ? 'selected' : '' }}>
                Aktif
              </option>
              <option value="Cuti" {{ request('sales_status') == 'Cuti' ? 'selected' : '' }}>
                Cuti
              </option>
              <option
                value="Nonaktif"
                {{ request('sales_status') == 'Nonaktif' ? 'selected' : '' }}>
                Nonaktif
              </option>
            </select>
          </div>
    ...
    <div
        ...
        <table class="min-w-full text-sm">
          <thead
            ...
            <tr>
              ...
              <th class="px-5 py-3 text-left font-semibold">Pencapaian</th>
          ...
          <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
            @forelse ($salesTeam as $sale)
              @php
                $achievement = $sale->achievement_this_month ?? 0;
                $target = $sale->target_sales > 0 ? $sale->target_sales : 0;
              ...
              <tr
                ...
                <td class="whitespace-nowrap px-5 py-4">
                  <div class="flex items-center">
                    <div class="h-10 w-10 flex-shrink-0">
                      <img
                        class="h-10 w-10 rounded-full border-2 border-slate-200 object-cover dark:border-slate-600"
                        src="https://ui-avatars.com/api/?name={{ urlencode($sale->name) }}&color=ffffff&background=3b82f6&bold=true"
                        alt="{{ $sale->name }}" />
                    </div>
                    <div class="ml-4">
                      ...
                      <div class="text-xs text-slate-500 dark:text-slate-400">
                        Bergabung
                        {{ \Carbon\Carbon::parse($sale->created)->isoFormat('D MMM YY') }}
                      </div>
                    </div>
                  </div>
                </td>
                <td class="whitespace-nowrap px-5 py-4">
                  <div class="mb-1 flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                    ...
                    <span class="text-xs font-medium">{{ $sale->email }}</span>
                  </div>
                  <div class="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                    ...
                    <span class="text-xs font-medium">{{ $sale->no_phone ?? '-' }}</span>
                  </div>
                </td>
                <td class="whitespace-nowrap px-5 py-4 text-center">
                  <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-2.5 py-1 text-[11px] font-bold text-blue-800 dark:bg-blue-500/20 dark:text-blue-400">
                    {{ $sale->customers_count }} pelanggan
                  </span>
                </td>
                <td class="whitespace-nowrap px-5 py-4">
                  ...
                  <div class="text-[11px] text-slate-500 dark:text-slate-400">
                    Tercapai: Rp {{ number_format($achievement, 0, ',', '.') }}
                  </div>
                </td>
                <td class="whitespace-nowrap px-5 py-4">
                  <div class="flex items-center">
                    <div class="mr-2 h-2 w-full rounded-full bg-slate-200 dark:bg-slate-700">
                      <div
                        class="{{ $percentage >= 100 ? 'bg-green-500' : ($percentage >= 75 ? 'bg-blue-500' : ($percentage >= 50 ? 'bg-amber-500' : 'bg-red-500')) }} h-2 rounded-full transition-all duration-300"
                        style="width: {{ min($percentage, 100) }}%"></div>
                    </div>
                    <span
                      class="whitespace-nowrap text-sm font-medium text-slate-800 dark:text-slate-100">
                ...
                <td class="whitespace-nowrap px-5 py-4">
                  @php
                    $statusColors = [
                      'Aktif' => 'bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-400',
                      'Cuti' => 'bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-400',
                      'Nonaktif' => 'bg-red-100 text-red-800 dark:bg-red-500/20 dark:text-red-400',
                    ];
                  @endphp

                  <span
                    class="{{ $statusColors[$sale->status] ?? 'bg-slate-100 text-slate-800 dark:bg-slate-600/20 dark:text-slate-300' }} inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold">
                    <span
                      class="{{ ['Aktif' => 'bg-green-500', 'Cuti' => 'bg-amber-500', 'Nonaktif' => 'bg-red-500'][$sale->status] ?? 'bg-slate-500' }} h-1.5 w-1.5 rounded-full"></span>
                    {{ $sale->status }}
                  </span>
                </td>
                <td class="whitespace-nowrap px-5 py-4 text-right text-xs font-medium">
                  <div class="flex items-center justify-end gap-2">
                    <a
                      href="{{ route('admin.sales.edit', $sale->id_sales) }}"
                    ...
                    <form
                      action="{{ route('admin.sales.destroy', $sale->id_sales) }}"
                      method="POST"
                      class="inline"
                      onsubmit="return confirm('Anda yakin ingin menghapus sales ini?');">
                      @csrf
                      @method('DELETE')
                      <button
                        type="submit"
                ...
                <td colspan="7" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center">
                    ...
                    <h3 class="mb-1 text-base font-semibold text-slate-600 dark:text-slate-300">
                      @if (request()->has('search_sales') || request()->has('sales_status'))
                        Tidak ada tim sales ditemukan
                      @else
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                      ...
                      @else
                          Mulai tambahkan sales pertama Anda
                      @endif
                    </p>
                  </div>
                </td>
      ...
      {{-- Pagination --}}
      @if (isset($salesTeam) && $salesTeam->hasPages())
        <div
          class="border-t border-slate-200 bg-slate-50 px-5 py-3 dark:border-slate-700 dark:bg-slate-800/50">
          {{ $salesTeam->appends(request()->query())->links('vendor.pagination.tailwind') }}
        </div>
      @endif
    </div>
  </div>
